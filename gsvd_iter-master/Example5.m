%% Accuracy of generalized right vectors of JBD-GSVD, by sin(\theta)
%
...
%%
% x1 = zeros(n,1);  % the aproximate first generalized right singular vector by using QR of C
% x2 = zeros(n,1);  % the aproximate first generalized  right singular vector by solving equation Cx=b
tol11 = 1e-10;  % tolerance1 of solving Cx=b
tol12 = 1e-8;  % tolerance2 of solving Cx=b
ang0 = zeros(k0,1);   % angle error of x1 using QR
ang1 = zeros(k0,1);   % angle error of x1 using tol1
ang2 = zeros(k0,1);   % angle error of x1 using tol2
...
for k = 1:k0
    ...
    ang1(k) = angle(X(:,1),x_k1);   % sin(angle) of right gen-singular vector x_k1
    ang2(k) = angle(X(:,1),x_k2);   % sin(angle) of right gen-singular vector x_k2
...
l = 1:k0;
figure;
semilogy(l,ang0,'->','Color','g','MarkerIndices',1:9:k0,...
'MarkerSize',8,'MarkerFaceColor','g','LineWidth',1.5);
hold on;
semilogy(l,ang1,'-o','Color','r','MarkerIndices',1:9:k0,...
'MarkerSize',8,'LineWidth',1.5);
hold on;
semilogy(l,ang2,'-D','Color','b','MarkerIndices',1:9:k0,...
'MarkerSize',8,'MarkerFaceColor','b','LineWidth',1.5);
hold on;
semilogy(l,bnd,'-','Color',[0.4940 0.1840 0.5560],'LineWidth',2);
handle = legend('$\sin\angle(x_{1},x_{1}^{(k)})$','$\sin\angle(x_{1},\bar{x}_{1}^{(k)})$',...
    '$\sin\angle(x_{1},\tilde{x}_{1}^{(k)})$','$\kappa(C)\tau/(c_{1}-c_{2})$',...
    'Location','northeast');
