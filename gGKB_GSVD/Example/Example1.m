%% Show the ghost generalized singular values phenomenon by gGKB-GSVD_A algorithm
...
for k = 1:k0
    ...
    B_k = B(1:k+1,1:k);
    c_k = svd(B_k);  
    H1(1:k, k) = c_k;  % approximate  gen singular values C (largest)
    H2(k0-k+1:k0, k) = c_k;  % approximate gen singular values C (smallest)
...
for i = 1: 3
    x_i = i : 1 : k0;
    plot(x_i, H1(i, i:k0), '-x', 'LineWidth', 1.0);
    hold on;
end
...
for i = 1: 3
    x_i = i : 1 : k0;
    plot(x_i, H2(k0-i+1, i:k0), '-x', 'LineWidth', 1.0);
    hold on;
end
% scatter(k0*ones(N, 1), s, 'ko');
xlabel('Iteration','Fontsize',15);
ylabel('Ritz value','Fontsize',15);
title('Approximations to $c_{i}$', 'Fontsize',17,'interpreter','latex');
...
for i = 1:k0 
    er1(i) = abs(c(1)-H1(1,i));   % error of c1
    er2(i) = abs(c(2)-H1(2,i));   % error of c2
    er3(i) = abs(c(3)-H1(3,i));   % error of c3
    
    er4(i) = abs(c(N)-H2(k0,i));  % error of s_N
    er5(i) = abs(c(N-1)-H2(k0-1,i));  % error of s_{N-1}
    er6(i) = abs(c(N-2)-H2(k0-2,i));  % error of s_{N-2}
end
...
handle=legend('$|c_{1}-\theta_{1}^{(k)}|$','$|c_{2}-\theta_{2}^{(k)}|$', '$|c_{3}-\theta_{3}^{(k)}|$','Location','northeast');
set(handle,'Fontsize',16,'interpreter','latex');
xlabel('Iteration','Fontsize',15);
ylabel('Error','Fontsize',15);
title('The first three $c_{i}$', 'Fontsize',17,'interpreter','latex');
grid on;
...
semilogy(l,er6,'-v','Color','m','MarkerIndices',1:9:k0,...
    'MarkerSize',5,'MarkerFaceColor','m','LineWidth',1.5);
handle=legend('$|c_{n}-\theta_{k}^{(k)}|$','$|c_{n-1}-\theta_{k-1}^{(k)}|$', '$|c_{n-2}-\theta_{k-2}^{(k)}|$','Location','northeast');
set(handle,'Fontsize',16,'interpreter','latex');
xlabel('Iteration','Fontsize',15);
ylabel('Error','Fontsize',15);
title('The last three $c_{i}$', 'Fontsize',17,'interpreter','latex');
grid on;
set(gca, 'GridAlpha', 0.2);
set(gca, 'MinorGridAlpha', 0.01);
